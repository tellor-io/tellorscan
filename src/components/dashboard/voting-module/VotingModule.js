...
const VoteItem = ({data}) => {
    ...
    useEffect(() => {
        if (currentUser) {
          currentUser.contracts.didVote(data.disputeId, currentUser.address)
    ...
    useEffect(() => {
        if (currentUser && data && (currentUser.address.toLowerCase() == data.miner.toLowerCase())) {
          setCantVote("You can't vote for own ETH address.");
          return
        }
        if (userBalance == 0) {
          setCantVote("You can't vote without holding TRB.");
          return
        } else {
          setCantVote()
        }
    ...
    return(
        <div className="VoteItem">
            ...
            <div className="dispvalue">
                <h3><span className="val">{getGranPrice(data.value,data.requestId)}</span> {data.requestSymbol}</h3>
            </div>
            <div className="otherinfo">
                ...
                <div>
                    ...
                    <h3><a>{truncateAddr(data.miner)}</a></h3>
                </div>
            </div>
            <div className="otherinfo">
                <div>
            ...
            {processing?
                ...
                <div className="buttons">
                    <Button
                        ...
                        disabled={cantVote}>
                            <CloseOutlined />challenge
                    </Button>
                    <Button
                        type="large"
                        onClick={() => handleSubmit(true, setCurrentTx1)}
                        disabled={cantVote}>
                            <CheckOutlined />support
                    </Button>
                </div>
            }
            </>
...
const VotingModule = ({disputes,activeDisputesCount,disputesReady}) => {
    const [copy,setCopy] = useState("There are currently no disputes to vote on.")
      ...
      if(activeDisputesCount){
        if(activeDisputesCount > 0){
            if(activeDisputesCount>1){
                setCopy("There are "+activeDisputesCount+" disputes to vote on.")
            } else {
                setCopy("There is 1 dispute to vote on.")
            }
        }
      }
    },[activeDisputesCount])
